 ...
 * Schedule sættes i netlify.toml: [functions.notify] schedule = "0 7 * * *"
 * Kører dagligt kl. 07:00 UTC (= 09:00 CEST).
 *
 * Kræver env-vars: SUPABASE_SERVICE_KEY + RESEND_API_KEY
 */
...
exports.handler = async () => {
  ...
  if (!SERVICE_KEY || !RESEND_KEY) {
    console.warn('[notify] Env vars mangler — notifikationer deaktiveret.');
  ...
  try {
    const profRes  = await fetch(`${SUPABASE_URL}/rest/v1/profiles?select=id,notification_frequency&notification_frequency=neq.none`, { headers: h });
    const profiles = await profRes.json();
    if (!Array.isArray(profiles) || profiles.length === 0) return { statusCode: 200, body: 'no subscribers' };
    ...
    for (const profile of profiles) {
      ...
      const subRes    = await fetch(`${SUPABASE_URL}/rest/v1/subscriptions?user_id=eq.${profile.id}&select=specialty`, { headers: h });
      ...
      const notesRes  = await fetch(`${SUPABASE_URL}/rest/v1/noter?select=id,title,category,created_at&or=(${catFilter})&created_at=gte.${since.toISOString()}&order=created_at.desc&limit=10`, { headers: h });
      const notes     = await notesRes.json();
      if (!Array.isArray(notes) || notes.length === 0) continue;

      const userRes  = await fetch(`${SUPABASE_URL}/auth/v1/admin/users/${profile.id}`, { headers: h });
      ...
      const noteRows    = notes.map(n =>
        `<tr><td style="padding:.6rem 0;border-bottom:1px solid #1a2740">
...
<div style="max-width:580px;margin:40px auto;background:#111827;border-radius:16px;overflow:hidden;border:1px solid rgba(14,165,197,0.2)">
  ...
  <div style="padding:32px">
    <div style="font-size:1.4rem;font-weight:700;color:#f0f4f8;margin-bottom:.25rem">${notes.length} nye kliniske cases</div>
    <div style="color:#8a9ab5;font-size:.875rem;margin-bottom:1.75rem">Nye noter ${periodLabel} i: ${specialties.join(', ')}</div>
    <table style="width:100%;border-collapse:collapse">${noteRows}</table>
    <div style="margin-top:1.75rem">
      <a href="${SITE_URL}" style="background:#0ea5c5;color:#fff;text-decoration:none;padding:.65rem 1.4rem;border-radius:8px;font-weight:600;font-size:.9rem;display:inline-block">Se alle cases →</a>
    </div>
  </div>
  <div style="padding:1rem 2rem;background:rgba(0,0,0,0.2);border-top:1px solid rgba(14,165,197,0.1)">
    <p style="margin:0;font-size:.78rem;color:#8a9ab5">Du modtager denne mail fordi du abonnerer på specialer på <a href="${SITE_URL}" style="color:#38bdf8">clinlog.dk</a>.</p>
  </div>
</div></body></html>`;
      ...
      const emailRes = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({ from: FROM_EMAIL, to: email, subject: `${notes.length} nye cases ${periodLabel} — Clinlog`, html }),
      });
      const emailData = await emailRes.json();
      console.log(`[notify] ${email}: ${emailRes.status}`, emailData?.id || emailData?.message || '');
      if (emailRes.ok) sent++;
    }
